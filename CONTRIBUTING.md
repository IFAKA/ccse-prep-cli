# Contributing

Thanks for helping improve CCSE Prep CLI.

## Development

```bash
npm install
npm test
npm run check:syntax
npm run audit:bank
npm run simulate:exams -- 100000
node bin/ccse.mjs --validate-bank
sh -n scripts/ccse-installer.sh
```

Keep changes focused and preserve the append-only event format. New behavior should have focused tests. Do not add unofficial exam questions, external content, AI-generated explanations, credentials, or local event data.

## Pull requests

Describe the user-visible behavior, the evidence used to validate it, and any compatibility impact. Keep generated files and unrelated formatting changes out of the diff.
